...
class AuctionManager:
    ...
    def __init__(self, num_players, required_count, num_artists, player_wealth=100,
        ...
        self.__server = Server(host, port, self.__player_count)
        self.__log = open('game_log.txt', 'w+')
    ...
    def run_game(self):
        ...
        for idx in range(len(self.players)):
            self.wealth_table[self.players[idx]['name']] = 100
        ...
        self.__server.update_all_clients(
            bytes(json.dumps({'artists_types': self.__artists_types,
                              ...
                              'wealth_table': self.wealth_table}), 'utf-8'), self.get_valid_players())
        ...
        while self.__game_state['remain_players'] > 0:
            ...
            return_state = {'finished': None, 'bid_item': None, 'winning_bid': None, 'bid_winner': None, 'wealth_table': dict(), 'remain_time': dict()}
            return_state['finished'] = game_state['finished']
            return_state['bid_item'] = game_state['bid_item']
            return_state['bid_winner'] = game_state['bid_winner']
            return_state['winning_bid'] = game_state['winning_bid']
            for idx in range(len(self.players)):
                name = self.players[idx]['name']
                if self.players[idx]['valid'] is True:
                    return_state['wealth_table'][name] = self.players[idx]['wealth']
...
#             bids = sorted(bids, key = itemgetter('received_time'))
            ...
            for idx in range(len(bids)):
                ...
                if self.players[player_id]['wealth'] - bid_summary['bid_amount'] >= 0:
                    ...
                    if bid_amt > max_bid['amount'] or (bid_amt == max_bid['amound'] and max_bid['amount'] >= 0 and
